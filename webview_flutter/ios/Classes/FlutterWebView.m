...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self = [super init]) {
    ...
    NSString* initialUrl = [self encodeToPercentEscapeString:args[@"initialUrl"]];
    if ([initialUrl isKindOfClass:[NSString class]]) {
      [self loadUrl:initialUrl];
    }
    
    [self getGestureRecognizer];
  }
  return self;
}
#pragma mark - 1.7.1增加事件解决卡顿问题
- (void)getGestureRecognizer{
    ...
    [_webView addGestureRecognizer:tap];
      ...
      UIPinchGestureRecognizer *pin= [[UIPinchGestureRecognizer alloc]initWithTarget:self action:@selector(pinmethod:)];
      pin.delegate = self;
...
-(void)tapmethod:(id)tap {
    [self fixFreeze:nil];
//    NSLog(@"点击了webview");
}
-(void)panmethod:(id)pan {
    [self fixFreeze:nil];
//    NSLog(@"拖动了webview");
}

-(void)pinmethod:(id)pin {
    [self fixFreeze:nil];
//    NSLog(@"缩放了webview");
}
...
- (void)onMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  } else if ([[call method] isEqualToString:@"fixFreeze"]) {
...
- (bool)loadRequest:(NSDictionary<NSString*, id>*)request {
  ...
  NSString* url = [self encodeToPercentEscapeString: request[@"url"] ];
